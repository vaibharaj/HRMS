...
class Appraisal(Document):
        ...
        if duplicate:
            frappe.throw(
                _(
                    ...
                    get_link_to_form("Appraisal", duplicate), frappe.bold(
                        self.employee_name)
                ),
    ...
    def set_kras_and_rating_criteria(self):
        ...
        template = frappe.get_doc(
            "Appraisal Template", self.appraisal_template)
        ...
        if self.rate_goals_manually:
            ...
            for entry in self.goals:
                if flt(entry.score) > 5:
                    frappe.throw(
                        _("Row {0}: Goal Score cannot be greater than 5").format(entry.idx))

                entry.score_earned = flt(
                    entry.score) * flt(entry.per_weightage) / 100
            ...
            self.goal_score_percentage = flt(
                goal_score_percentage, self.precision("goal_score_percentage"))
    ...
    def calculate_avg_feedback_score(self, update=False):
        ...
        self.avg_feedback_score = flt(
            avg_feedback_score, self.precision("avg_feedback_score"))
    ...
    def calculate_final_score(self):
        final_score = (flt(self.total_score) +
                       flt(self.avg_feedback_score) + flt(self.self_score)) / 3
    ...
    def set_goal_score(self, update=False):
        for kra in self.appraisal_kra:
            ...
            kra.goal_completion = flt(
                avg_goal_completion, kra.precision("goal_completion"))
            kra.goal_score = flt(
                kra.goal_completion * kra.per_weightage / 100, kra.precision("goal_score"))
...
def get_feedback_history(employee, appraisal):
    ...
    data.feedback_history = frappe.get_list(
        ...
        filters={"employee": employee,
                 "appraisal": appraisal, "docstatus": 1},
        fields=[
            "feedback",
            "reviewer",
            "user",
    ...
    for i in range(1, 6):
        ...
        percent = flt((count / feedback_count) * 100,
                      0) if feedback_count else 0
        reviews_per_rating.append(percent)

    data.reviews_per_rating = reviews_per_rating
    data.avg_feedback_score = frappe.db.get_value(
        "Appraisal", appraisal, "avg_feedback_score")
