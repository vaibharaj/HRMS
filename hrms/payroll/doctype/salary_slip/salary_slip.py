...
class SalarySlip(TransactionBase):
    ...
    def payroll_period(self):
        if not hasattr(self, "_payroll_period"):
            self._payroll_period = get_payroll_period(
    ...
    def validate_dates(self, joining_date=None, relieving_date=None):
        ...
        if date_diff(self.end_date, joining_date) < 0:
            frappe.throw(
                _("Cannot create Salary Slip for Employee joining after Payroll Period"))
    ...
    def compute_income_tax_breakup(self):
        ...
        if hasattr(self, "total_structured_tax_amount") and hasattr(
            ...
            self.total_income_tax = self.income_tax_deducted_till_date + \
                self.future_income_tax_deductions
    ...
    def get_amount_from_formula(self, struct_row, sub_period=1):
        if self.payroll_frequency == "Monthly":
            ...
            posting_date = frappe.utils.add_months(
                self.posting_date, sub_period)
        ...
        try:
            ...
            if struct_row.amount_based_on_formula:
                ...
                if formula:
                    amount = flt(
                        frappe.safe_eval(
                            formula, self.whitelisted_globals, data), struct_row.precision("amount")
                    )
            if amount:
    ...
    def update_component_row(
        ...
        self.update_component_amount_based_on_payment_days(
            component_row, remove_if_zero_valued)
    ...
    def calculate_variable_tax(self, tax_component):
        ...
        if self.current_additional_earnings_with_full_tax:
            ...
            self.full_tax_on_additional_earnings = self.total_tax_amount - \
                self.total_structured_tax_amount

        current_tax_amount = self.current_structured_tax_amount + \
    ...
    def get_income_tax_slabs(self):
        ...
        if income_tax_slab_doc.disabled:
            frappe.throw(
                _("Income Tax Slab: {0} is disabled").format(income_tax_slab))
    ...
    def get_salary_slip_details(
        ...
        if exempted_from_income_tax:
            query = query.where(sd.exempted_from_income_tax ==
                                exempted_from_income_tax)
        ...
        if allow_tax_exemption:
            for ded in self.deductions:
                if ded.exempted_from_income_tax:
                    ...
                    additional_income -= additional_amount
                    amount_exempted_from_income_tax = flt(
    ...
    def calculate_unclaimed_taxable_benefits(self):
        ...
        total_benefits_claimed = flt(
            total_benefits_claimed[0][0]) if total_benefits_claimed else 0
        ...
        if self.tax_slab.allow_tax_exemption:
            if self.deduct_tax_for_unsubmitted_tax_exemption_proof:
                exemption_proof = frappe.db.get_value(
                    ...
                    {"employee": self.employee,
                     "payroll_period": self.payroll_period.name, "docstatus": 1},
                    ["exemption_amount"],
                )
                if exemption_proof:
    ...
    def get_loan_details(self):
        loan_details = frappe.get_all(
            "Loan",
            fields=["name", "interest_income_account",
                    "loan_account", "loan_type", "is_term_loan"],
            filters={
...
def eval_tax_slab_condition(condition, eval_globals=None, eval_locals=None):
    ...
    try:
        ...
        frappe.throw(
            _("Syntax error in condition: {0} in Income Tax Slab").format(err))
    except Exception as e:
        frappe.throw(
            _("Error in formula or condition: {0} in Income Tax Slab").format(e))
        raise
...
def set_missing_values(time_sheet, target):
    ...
    target.append(
        "timesheets", {"time_sheet": doc.name, "working_hours": doc.total_hours})
